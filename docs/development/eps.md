...
- **Switched versus always-on.** A very small always-on domain – the EPS itself, the watchdog, the [inhibit](inhibits-hdrm.md) logic, the RTC – plus everything else on switchable channels. The always-on domain is your last line of defence: it must be simple enough that you are confident it cannot fail.
