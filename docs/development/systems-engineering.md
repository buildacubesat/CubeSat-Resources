...
- **Team size and skills** is an architectural driver that formal methods rarely acknowledge. A four-person team should not architect a system that needs eight people to build.
...
- **Volume budget**: less rigorous than the others, but useful for tracking how much room is left after structure, harnessing, and mechanical clearances are accounted for. Especially easy to underestimate on dense 1U or 1.5U designs.
...
Budgets are living documents. Update them after every major design decision, every test that produces measured numbers, and every time a vendor delivers a component with different specs than the datasheet advertised. A budget that hasn't moved in months is either a finished design or, more often, an abandoned one.
...
5. **Hardware redundancy** – duplication, which costs mass, power and the switching mechanism.

The CubeSat position is that **redundancy is usually the wrong answer and better testing is usually the right one.** Mass and volume are too scarce for meaningful duplication, and the observed failure modes are largely ones testing would have caught.
