...
- **The deployer sets the travel, and the CDS does not.** CDS §2.3.2.3 only requires an actuated switch to sit at or below the interfacing surface; how far it has to move to actuate is the deployer's business, and the number constrains which switch part you can use. XTERRA's XCD guide asks for at least 0.75 mm of activation travel on switches at the rail ends in Z, and at least 1.5 mm on switches along the rail sides in X or Y, and will supply a simplified dispenser CAD model for a digital fit check before any hardware exists.[^xcd-guide] Get the equivalent figure from your own deployer's manual early – it is far cheaper to find out before the rails are machined.
...
[^xcd-guide]: XTERRA, *XCD-3U/6U Hybrid User's Guide*, document XTD-100084 Revision 1.3.3, 21 January 2026, available from [xterra.space](https://www.xterra.space/products). Free PDF. Section 4.5 places activation switches at the rail ends in Z or along the rail sides in X or Y, recommends a minimum activation travel of 0.75 mm for the former and 1.5 mm for the latter, and offers a simplified dispenser CAD model for a digital fit check. Cited here as a published example of a deployer stating switch requirements the CDS leaves open; confirm the figures for the dispenser and revision your mission actually flies in.
