...
For compression, [CCSDS](../references/glossary.md#ccsds) publishes two standards, and which one applies depends on your instrument:

- **CCSDS 122.0-B-2**, "Image Data Compression", covers two-dimensional single-band imagery – frame-based data from CCD arrays and strip-based data from push-broom sensors – and "can be used to produce both lossy and lossless compression".[^ccsds122] This is the one that applies to an ordinary camera, which is what most CubeSats actually carry.
- **CCSDS 123.0-B-2**, "Low-Complexity Lossless and Near-Lossless Multispectral and Hyperspectral Image Compression", handles three-dimensional multispectral and hyperspectral arrays, in both lossless mode and a near-lossless mode where the maximum reconstruction error is bounded to a user-specified value.[^ccsds123] A bounded-error mode is often exactly what a scientist can accept when unbounded lossy compression is not.
...
- **[CCSDS 122.0-B-2: Image Data Compression](https://ccsds.org/Pubs/122x0b2e1.pdf)** `PDF` – CCSDS standard for lossy and lossless compression of two-dimensional single-band imagery, frame-based or push-broom. The one that applies to an ordinary CubeSat camera
...
[^eirsat-gmod]: David Murphy et al., ["A compact instrument for gamma-ray burst detection on a CubeSat platform II: Detailed design, assembly and validation"](https://arxiv.org/abs/2203.03502), *Experimental Astronomy*, 2022 (preprint arXiv:2203.03502; PubMed PMID 35795408). Describes EIRSAT-1's GMOD instrument: a 25 × 25 × 40 mm cerium bromide scintillator read out by a 4×4 tiled silicon photomultiplier array, achieving 5.4% energy resolution at 662 keV, with electrical and mechanical interfaces deliberately made compatible with off-the-shelf CubeSat systems.

[^ccsds122]: Consultative Committee for Space Data Systems, [*Image Data Compression*, CCSDS 122.0-B-2](https://ccsds.org/Pubs/122x0b2e1.pdf), Recommended Standard Issue 2, September 2017. Applies to two-dimensional digital spatial image data from spacecraft instruments, covering frame-based formats from CCD arrays and strip-based formats from push-broom sensors, and states that the technique "can be used to produce both lossy and lossless compression".

[^ccsds123]: Consultative Committee for Space Data Systems, [*Low-Complexity Lossless and Near-Lossless Multispectral and Hyperspectral Image Compression*, CCSDS 123.0-B-2](https://ccsds.org/Pubs/123x0b2e2c3.pdf), Recommended Standard Issue 2, February 2019, incorporating Technical Corrigendum 3 (February 2021) and subsequent editorial changes. Defines lossless and near-lossless compression for three-dimensional multispectral and hyperspectral arrays, in "a way that the maximum error in the reconstructed image can be limited to a user-specified bound", with three entropy coding options (sample-adaptive, hybrid and block-adaptive).