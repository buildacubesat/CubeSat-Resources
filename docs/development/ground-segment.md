...
- **Authentication.** An unauthenticated command link means anyone with a transmitter and your published format can command your spacecraft. Amateur regulations complicate encryption in some jurisdictions — command authentication (proving the sender's identity) is usually distinguishable from encryption (hiding the content) and is worth understanding for your regulatory regime. See [Regulatory and Operational Considerations](#regulatory-and-operational-considerations).
<!-- NEEDS HUMAN REVIEW: The legality of authenticating (vs. encrypting) amateur-band satellite commands varies by national regulator and is genuinely contested. I have deliberately kept this general rather than giving guidance that might be wrong in a given country. A pointer to a specific authoritative source here would strengthen the page. -->
...
- **Amateur service constraints** typically include a prohibition on commercial use and restrictions on encryption, and require that communications be open. These are not obstacles for an educational mission but they must be understood before choosing the band.
- **Power and duty cycle limits** apply to your ground transmitter under your licence.
- **Coordination.** The [IARU](https://www.iaru.org/on-the-air/satellites/) coordinates amateur satellite frequencies to avoid interference; going through this process is expected practice.
...
- **[Gpredict](https://oz9aec.dk/gpredict/)** `Link` – Mature satellite tracking software with Doppler correction and Hamlib rotator support
...
- **Hamlib (`rotctl` / `rotctld`)** – the standard Linux backend for antenna rotator control, offering CLI tools and a network daemon. Its companion `rigctl`/`rigctld` does the same for radios.
- **grotor** – simple GTK GUI frontend for Hamlib, useful for manual az/el control.
- **gr-rotator (GNU Radio block)** – enables programmatic rotator control directly from signal-processing flows.
